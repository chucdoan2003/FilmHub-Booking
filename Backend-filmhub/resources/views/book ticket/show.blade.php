...
<div class="container">
    <form action="{{ route('vnpay_payment') }}" method="POST" id="payment-form">
        @csrf
        <input type="hidden" name="showtime_id" value="1"> <!-- Giá trị của showtime_id -->
        <input type="hidden" name="user_id" value="1"> <!-- Thay đổi giá trị này cho phù hợp với người dùng hiện tại -->
        <input type="hidden" id="selected_seats" name="selected_seats" value="">

        <h1>Thông tin phim</h1>
        <div>
            <h2>{{ $showtime->movie->title }}</h2>
            <img src="{{ Storage::url($showtime->movie->poster_url) }}" style="width: 200px; height: auto;" alt="Poster">
            ...
            <p><strong>Thời gian chiếu:</strong>
                @if ($startTime->format('Y-m-d') === $endTime->format('Y-m-d'))
                    {{ $startTime->format('d/m/Y') }}: {{ $startTime->format('H:i') }} - {{ $endTime->format('H:i') }}
                @else
                    {{ $startTime->format('d/m/Y') }} - {{ $endTime->format('d/m/Y') }}: {{ $startTime->format('H:i') }} - {{ $endTime->format('H:i') }}
                @endif
            </p>
        ...
        <div class="seat-selection">
            ...
            <div class="seat">
                <input type="checkbox" id="seat-{{ $seat->seat_id }}" value="{{ $seat->seat_id }}" data-price="{{ $showtime->value }}">
                <label for="seat-{{ $seat->seat_id }}">
                    {{ $seat->number }} (Số: {{ $seat->seat_id }}) <!-- Hiển thị ID ghế -->
                </label>
            </div>
            @endforeach
        </div>
...
<script>
    ...
    document.getElementById('selected-seats-display').innerText = selectedSeatIds.join(', ') || 'Chưa có ghế nào được chọn';
}
