                            ...
                            @if (isset($users))
                                @foreach ($users as $item)
                                    <tr>
                                        ...
                                        <td><img src="" alt=""></td>
                                        <td>
                                            <a href="{{ route('users.edit', $item->id) }}"><button class="btn-warning my-2">Edit</button></a>
                                            <form id="deleteForm" action="{{ route('users.destroy', $item->id) }}" method="POST" >
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('Do you want to delete this user ?')" class="btn-danger">Delete</button>
                                            </form>
