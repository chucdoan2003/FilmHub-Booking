                    ...
                    <select name="movie" id="" disabled>
                        @foreach ($movies as $item)
                            <option value="{{ $item->movie_id }}"
                                @if ($item->movie_id == $movie_id) @selected(true) @endif>{{ $item->title }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="room" id="" disabled>
                        @foreach ($rooms as $item)
                            <option value="{{ $item->room_id }}"
                                @if ($item->room_id == $room_id) @selected(true) @endif>{{ $item->room_name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="shift" id="">
                        @foreach ($shifts as $item)
                            <option value="{{ $item->shift_id }}"
                                @if (in_array($item->shift_id, $shiftInroomBook)) @disabled(true) @endif>
