			...
			<div class="container container_seat">
				<div class="row">
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="st_bt_top_center_heading st_bt_top_center_heading_seat_book_page float_left">
							...
							<h4> 
								...
								Ca chiếu: {{ $showtime->shifts->shift_name }} : {{ $shiftStartTime->format('H:i') }} - {{ $shiftEndTime->format('H:i') }}
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						...
						<div class="st_seatlay_btn float_left"> <button type="submit" class="btn btn-success">Proceed to Pay</button>
						</div>
					</div>
				</div>
			</div>
							...
							<div class="st_seat_lay_row float_left">
								<ul>
									...
									@foreach($row->seats as $seat)
										<li>	
											<span>{{ $seat->types->type_name == 'Ghế thường' ? $showtime->normal_price : $showtime->vip_price }} VNĐ</span>
											<input type="checkbox" id="c{{$seat->seat_id}}" name="seats[]" value="{{ $seat->seat_number }}" class="seat-checkbox" data-price="{{ $seat->types->type_name == 'Ghế thường' ? $showtime->normal_price : $showtime->vip_price }}">
											<label for="c{{$seat->seat_id}}" class="seat-label" data-seat-number="{{ $seat->seat_number }}"></label>
										</li>
									@endforeach
								</ul>
							</div>
