...
class ClientBookingController extends Controller
{
    ...
    public function getSeatBooking($showtime_id)
    {
        ...
        if ($timeUntilShowtime < 10) {
            // Không cho đặt vé nếu còn dưới 10 phút
            return redirect()->route('movies.index')->with('error', 'Không thể đặt vé, vui lòng chọn ca chiếu khác.');
        }

        if ($timeUntilShowtime >= 15) {
            // Cho phép đặt vé bình thường nếu còn trên 15 phút
            $bookingTimeLimit = null; // Không giới hạn
        } else {
            // Giới hạn thời gian đặt vé còn 5 phút nếu trong khoảng 10-15 phút
            $bookingTimeLimit = 5 * 60; // Tính bằng giây
            session(['booking_start_time' => now()]);
        }
        ...
        return view('frontend.layouts.booking.getSeatBooking', compact('showtime', 'user_id', 'bookedSeats', 'bookingTimeLimit'));
    }
