...
public function getSeatBooking($showtime_id)
{
    ...
    // Thời gian còn lại tính từ hiện tại
    $timeRemaining = $currentDateTime->diffInSeconds($showtimeStart, false) - 600; // 10 phút trừ đi thời gian đến giờ chiếu
    if ($timeRemaining <= 0) {
        return redirect()->route('movies.index')->with('error', 'Sắp đến giờ chiếu, vui lòng chọn ca chiếu khác.');
    }
    ...
    return view('frontend.layouts.booking.getSeatBooking', compact('showtime', 'user_id', 'bookedSeats', 'timeRemaining'));
}
