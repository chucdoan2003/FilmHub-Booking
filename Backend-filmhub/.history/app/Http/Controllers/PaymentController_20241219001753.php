...
class PaymentController extends Controller
{
    public function vnpay_payment(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->withErrors(['errorerror' => 'Lỗi khi truy vấn mã giảm giá: ' . $e->getMessage()]);
        }
        if ($vourcher) {
    ...
    public function vnpay_payment_return(Request $request)
    {
        ...
        if ($paymentStatus != '00') {
            ...
            if (!$ticket) {
                ...
                return redirect()->route('movies.index')->with('error', 'Bạn đã hủy thanh toán.');
            } else {
        ...
        if ($user) {
            ...
            if ($discountCode) {
                ...
                if ($voucher) {
                    ...
                    \DB::table('vourchers_redeem')
                    ->where('id', $discountCode)  // Điều kiện mã giảm giá
                    ->where('user_id', $userId)   // Điều kiện user_id
                    ->delete();
