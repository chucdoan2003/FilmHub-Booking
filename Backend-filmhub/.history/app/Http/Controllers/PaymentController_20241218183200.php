...
class PaymentController extends Controller
{
    public function vnpay_payment(Request $request)
    {
        ...
        try {
            ...
            // if ($vourcher->end_time <= now()) {
            // return redirect()->back()->withErrors(['msg' => 'Mã voucher đã hết hạn. Vui lòng chọn mã khác.']);
            // }
            if ($vourcher->end_time <= now()) {
                return redirect()->back()->withErrors(['error' => 'Mã voucher đã hết hạn. Vui lòng chọn mã khác.'], 400);
            }
        } catch (\Exception $e) {
            return redirect()->back()->withErrors(['error' => 'Lỗi khi truy vấn mã giảm giá: ' . $e->getMessage()]);
        }
        if ($vourcher) {
