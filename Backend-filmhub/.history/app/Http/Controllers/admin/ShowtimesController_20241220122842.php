...
class ShowtimesController extends Controller
{
    ...
    function create(){
        ...
        $rooms = DB::table('rooms')->where('theater_id', Auth::user()->theater_id)->get();
    ...
    function edit(string $id){
        ...
        $ticket = DB::table('tickets')->where('showtime_id', $id)->first();
        if(!empty($ticket)){
            $not_edit = 'Ca chiếu đã có vé đặt, bạn không thể sửa';
            $showtimes = DB::table('showtimes')
                ->latest()
                ->join('movies', 'showtimes.movie_id', '=', 'movies.movie_id')
                ->join('rooms', 'showtimes.room_id', '=', 'rooms.room_id')   // Join với bảng rooms
                ...
                return view('admin.showtimes.list', compact('showtimes', 'not_edit'));
        }
        $showtime = DB::table('showtimes')->where('showtime_id', $id)->first();
        $showtime_id =$id;
        ...
        $inputDate = Carbon::createFromFormat('Y-m-d', $showtime->datetime , 'Asia/Ho_Chi_Minh');
        $currentDate = Carbon::now();
        if($inputDate->isPast() || $inputDate->isToday())
        {
            $not_edit = 'Không được sửa xuất chiếu của ngày đã diễn ra, hoặc trong ngày hôm nay';
            $showtimes = DB::table('showtimes')
                ->latest()
                ->join('movies', 'showtimes.movie_id', '=', 'movies.movie_id')
                ->join('rooms', 'showtimes.room_id', '=', 'rooms.room_id')   // Join với bảng rooms
