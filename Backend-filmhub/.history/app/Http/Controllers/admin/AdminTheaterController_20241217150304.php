    ...
    public function destroy(Theater $theater)
{
    ...
    // Debug: Kiểm tra tất cả showtimes của rạp
    $showtimes = Showtime::where('theater_id', $theater->theater_id)->get();
    dd($showtimes);

    // Kiểm tra xem có showtime nào chưa chiếu hoặc đang chiếu không
    $hasActiveShowtimes = Showtime::where('theater_id', $theater->id)
        ->where(function ($query) use ($currentDateTime) {
            $query->where('datetime', '>', $currentDateTime->toDateString())
                  ->orWhere(function ($subQ) use ($currentDateTime) {
                      $subQ->where('datetime', $currentDateTime->toDateString())
                          ->whereHas('shift', function ($shiftQ) use ($currentDateTime) {
                              $shiftQ->where('start_time', '<=', $currentDateTime->toTimeString())
    ...
    if ($hasActiveShowtimes) {
        return redirect()->route('admin.theaters.index')->with('error', 'Không thể xóa rạp vì có ca chiếu chưa hoặc đang diễn ra.');
    }
    // dd( $hasActiveShowtimes);
