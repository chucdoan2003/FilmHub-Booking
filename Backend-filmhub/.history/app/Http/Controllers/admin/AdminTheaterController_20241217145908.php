    ...
    $hasActiveShowtimes = Showtime::where('theater_id', $theater->id)
        ->where(function ($query) use ($currentDateTime) {
            $query->where(function ($q) use ($currentDateTime) {
                // Lấy start_time và end_time từ bảng shifts thông qua showtime
                $q->where('datetime', '>', $currentDateTime->toDateString()) // Ngày chiếu trong tương lai
                  ->orWhere(function ($subQ) use ($currentDateTime) {
                      $subQ->where('datetime', $currentDateTime->toDateString()) // Ngày chiếu hôm nay
                          ->whereHas('shiftss', function ($shiftQ) use ($currentDateTime) {
