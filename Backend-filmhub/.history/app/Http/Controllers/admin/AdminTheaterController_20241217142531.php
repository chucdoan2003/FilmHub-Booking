    ...
    public function destroy(Theater $theater)
{
    try {
        ...
        // Kiểm tra xem có ca chiếu nào chưa chiếu không
        $hasUpcomingShowtimes = $theater->shifts()
            ->where('start_time', '>', $currentDateTime->toTimeString()) // Ca chiếu chưa diễn ra
            ->exists();

        if ($hasUpcomingShowtimes) {
            DB::rollBack();
            return redirect()->route('admin.theaters.index')->with('error', 'Không thể xóa rạp vì có ca chiếu chưa diễn ra.');
        }

        // Xóa các phòng và các ca chiếu liên quan
        $theater->rooms()->delete();
        $theater->shifts()->delete();
