                                        ...
                                        <div class="st_cherity_img_cont float_left">
                                            <div class="box">
                                                <div class="mb-4">
                                                    ...
                                                    <select id="combo" name="combo_id[]" multiple
                                                        class="selectpicker form-control" onchange="updateTotalPrice()"
                                                        data-live-search="true">
                                                        @foreach ($combos as $combo)
                                                        <option value="{{ $combo->id }}" data-price="{{ $combo->price }}">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <select id="food" name="food_id[]" multiple
                                                        class="selectpicker form-control" onchange="updateTotalPrice()"
                                                        data-live-search="true">
                                                    ...
                                                    <select id="discount_code" name="discount_code" class="form-control"
                                                        ...
                                                        <optgroup label="Mã giảm giá sự kiện">
                                                            ...
                                                            <option value="{{ $event->vourcher_code }}">
                                                                {{ $event->vourcher_code }} ({{ $event->discount_percentage
                                                                }}% giảm, tối đa {{ $event->max_discount_amount }} VNĐ)
                                                            </option>
                                                            @endif
                                                            @endforeach
                                                        </optgroup>
...
<script>
    function updateTotalPrice() {
        var $totalAmount = {{ $totalAmount}}; // Giá hiện tại
        ...
        $totalAmount += comboPrice; // Cộng giá combo vào tổng

        // Cộng giá món ăn
        if (foodSelect.value) {
            var selectedFood = foodSelect.options[foodSelect.selectedIndex];
            t$totalAmount += parseFloat(selectedFood.dataset.price || 0);
        }

        // Cộng giá đồ uống
        if (drinkSelect.value) {
            var selectedDrink = drinkSelect.options[drinkSelect.selectedIndex];
            $totalAmount += parseFloat(selectedDrink.dataset.price || 0);
        }
        ...
        if (discountCode && discounts[discountCode]) {
            ...
            if ($totalAmount - discountAmount < 0) {
                discountAmount = $totalAmount; // Áp dụng giảm tối đa đến giá trị hiện tại
            }
