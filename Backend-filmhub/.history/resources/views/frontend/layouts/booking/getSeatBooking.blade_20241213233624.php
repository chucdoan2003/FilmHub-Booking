...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    let serverStartTime = "{{ session('booking_start_time') }}"; // Lấy thời gian bắt đầu từ server
    let bookingDuration = 5 * 60; // Giới hạn 5 phút (300 giây)
    let timerElement = document.getElementById('timer');

    if (serverStartTime) {
        let startTime = new Date(serverStartTime).getTime();
        let currentTime = new Date().getTime();
        let elapsedTime = Math.floor((currentTime - startTime) / 1000); // Thời gian đã trôi qua

        let remainingTime = bookingDuration - elapsedTime;
