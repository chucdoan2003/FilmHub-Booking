...
<form action="{{ route('vnpay_payment') }}" method="post" enctype="multipart/form-data">
    ...
    <input type="text" name="selected_seats" value="{{ implode(',', $selectedSeats2->pluck('seat_id')->toArray()) }}">
                                            ...
                                            <span class="dtts2">
                                                @if ($selectedSeats2->isNotEmpty())
                                                    @foreach ($selectedSeats2 as $seat)
                                                        <span class="selected-seat" data-seat-id="{{ $seat->seat_id }}">
                                                            {{ $seat->seat->seat_number }} <button type="button" class="remove-seat-btn">Xóa</button>
                                                        </span>
                                                    @endforeach
                                                @else
                                                    ...
                                                    <optgroup label="Mã giảm giá sự kiện">
                                                        @foreach($vourcherEvents as $event)
                                                            ...
                                                            <option value="{{ $event->vourcher_code }}">
                                                                {{ $event->vourcher_code }} ({{ $event->discount_percentage }}% giảm, tối đa {{ $event->max_discount_amount }} VNĐ) (Mã sự kiện)
                                                            </option>
                                                            @endif
                                                        @endforeach
                                                    </optgroup>
...
<script>
   document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectedSeatsDisplay = document.querySelector('.dtts2');

    // Hàm để cập nhật giá trị input selected_seats
    function updateSelectedSeats() {
        const selectedSeats = [];
        document.querySelectorAll('.selected-seat').forEach(function (seat) {
