                ...
                <div class="st_seat_full_container">
                    <div class="st_seat_lay_economy_wrapper ">
                        ...
                        @foreach ($showtime->rooms->rows as $row)
                            ...
                            @foreach ($seatChunks as $index => $chunk)
                                <div class="st_seat_lay_row float_left">
                                    <ul>
                                        <li class="st_seat_heading_row {{ $index !== 0 ? 'hidden' : '' }}">{{ $row->row_name }}</li>
    ...
    <script>
        document.querySelector("form").addEventListener("submit", function(event) {
            ...
            if (selectedSeats === "" || selectedSeats === null) {
                // Hiển thị cảnh báo SweetAlert2 thay thế alert
                Swal.fire({
                    icon: 'warning',
                    title: 'Chưa chọn ghế!',
                ...
                return; // Dừng lại không gửi form nếu không chọn ghế
            }
