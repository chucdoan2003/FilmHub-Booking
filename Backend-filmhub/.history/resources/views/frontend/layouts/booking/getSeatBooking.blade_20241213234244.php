...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    let bookingTimeLimit = {{ $bookingTimeLimit ?? 'null' }}; // Lấy thời gian tối đa từ backend
    let timerElement = document.getElementById('timer');

    // Nếu có thời gian còn lại trong sessionStorage, lấy về
    if (sessionStorage.getItem(timerKey)) {
        bookingTimeLimit = parseInt(sessionStorage.getItem(timerKey), 10);
    }

    // Nếu có thời gian đặt, bắt đầu đếm ngược
    if (bookingTimeLimit) {
